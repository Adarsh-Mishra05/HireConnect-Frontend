...
export class ProfileComponent implements OnInit {
    ...
    this.profileService.getMyProfile().subscribe({
      next: (response) => {
        ...
        if (this.profile?.phone) {
          this.authStorage.setUserPhone(this.profile.phone);
        }
  ...
  buildEditableProfile(source: any): any {
    const seeded = this.getSeededProfileData();

    return {
      ...source,
      firstName: source?.firstName || seeded.firstName,
      lastName: source?.lastName || seeded.lastName,
      phone: source?.phone || seeded.phone,
      email: seeded.email,
  ...
  saveProfile(): void {
    ...
    request$.subscribe({
      next: (response) => {
        ...
        const profileFullName = `${this.profile?.firstName || ''} ${this.profile?.lastName || ''}`.trim();
        if (profileFullName) {
          this.authStorage.setUserFullName(profileFullName);
        }
        this.profileExists = true;
        this.editableProfile = this.buildEditableProfile(this.profile);
  ...
  get fullName(): string {
    ...
    return profileName || this.authStorage.getUserFullName() || 'Your Name';
  }
  ...
  get initials(): string {
    const first = this.profile?.firstName?.charAt(0)
      || this.authStorage.getUserFullName()?.charAt(0)
      || '';
    const last = this.profile?.lastName?.charAt(0)
      || this.authStorage.getUserFullName()?.split(' ')[1]?.charAt(0)
      || '';
  ...
  private getSeededProfileData(): { firstName: string; lastName: string; phone: string; email: string } {
    const fullName = (this.authStorage.getUserFullName() || '').trim();
    const phone = this.authStorage.getUserPhone() || '';
    const email = this.authStorage.getUserEmail() || '';
