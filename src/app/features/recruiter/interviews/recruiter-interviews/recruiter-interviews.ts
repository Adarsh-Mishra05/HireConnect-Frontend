...
export class RecruiterInterviewsComponent implements OnInit {
  ...
  async acceptReschedule(interviewId: number): Promise<void> {
    const confirmed = await this.confirmModal.open({
      title: 'Accept Reschedule',
      message: 'Are you sure you want to accept the new scheduled time for this interview?',
      confirmText: 'Accept',
      cancelText: 'Cancel',
      variant: 'primary'
    });
    ...
    this.interviewService.updateInterview(interviewId, { status: 'SCHEDULED' }).subscribe({
      next: () => {
        this.interviews = this.interviews.map(interview =>
          interview.id === interviewId
        ...
        this.toastService.show('Reschedule accepted successfully', 'success');
        this.loadInterviews();
      },
      error: (error) => {
        const message = error?.error?.message || 'Failed to accept reschedule';
        this.toastService.show(message, 'error');
      }
    });
  }
